...
bigint* invmod_ (BI_CTX *ctx, bigint *a, bigint *b)
{
  bigint *mod = b;
  bigint *A, *B, *C, *D, *zero, *one;
  int _a = 0, _b = 0, _c = 0, _d = 0, low;
  A = int_to_bi(ctx, 1);
  B = int_to_bi(ctx, 0);
  C = int_to_bi(ctx, 0);
  D = int_to_bi(ctx, 1);
  zero = int_to_bi(ctx, 0);
  one = int_to_bi(ctx, 1);
  ...
  {
    a = trim(a);
    if (bi_compare(a, zero) <= 0)
      break;
    q = bi_divide(ctx, b, a, 0);
    bi_permanent(q);
    x = bi_multiply(ctx, q, a);
    x = bi_subtract(ctx, b, x, 0);
    b = bi_copy(a);
    if (b->refs < 5)
      bi_permanent(b);
    a = x;
...
//u_a, v_a, u_b, v_b = [ u_b - q*u_a, v_b - q*v_a, u_a, v_a ]
    _A = bi_multiply(ctx, A, q);
    _B = bi_multiply(ctx, B, q);

    x = bi_subtract(ctx, C, _A, &low);
    if (_a ^ _c)
    {
      bi_free (ctx, x);
      x = bi_add(ctx, C, _A);
  ...
  if (bi_compare(b, one) == 0)
    return 0;

  return bi_divide(ctx, C, mod, 1);
}

bigint* invmod__ (BI_CTX *ctx, bigint *a, bigint *b)
{
